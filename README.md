...
五 支持向量机
    ...
    SMO算法原理：每次循环中选择两个alpha进行优化处理。一旦找到一对合适的alpha，那么就增大其中一个同时减小另一个。这里所谓的"合适"就是指两个alpha必须要符合一定的条件，条件之一就是这两个
    alpha必须要在间隔边界之外，第二个条件是这两个alpha还没有进行过区间化处理或者不在边界上。
    SMO伪码：
...
| SVD  | 简化数据，去除噪声，提高算法的结果     | 数据的转换可能难以理解         | 数值型数据   |
	...
	如果选择前r个奇异值了？一个典型的做法是保留矩阵中90%的能量信息。另一个启发策略就是当矩阵中有上万个奇异值时，保留前2000或3000个。
	...
	推荐引擎评价的指标是称为最小均方根误差的指标，使用交叉验证求误差，集将某些已知的评分去掉，然后对他们进行预测，最后计算预测值和真实值之间的差异。
